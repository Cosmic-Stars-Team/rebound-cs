...
> 将 REBOUNDx 的物理扩展模块直接内嵌进 REBOUND，去除 Python 依赖，支持 MSVC 编译，面向 rebound-rs的rust封装(https://github.com/Cosmic-Stars-Team/rebound-rs)。
...
REBOUNDx 在 MSVC 上编译失败，导致无法直接集成到 Cosmic Stars 项目。
---
...
本项目的存在意义是为了在特定的工程约束下继续使用它们出色的物理实现。
所有代码遵循 GPL v3 协议。
